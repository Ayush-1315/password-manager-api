...
const {transporter,userDeleteMailOptions, userDeletedMailOptions, welcomeMailOptions,generatePasswordResetMailOptions}=require('../utils/mailManager');
...
const sendPasswordResetOtp=(email,username,res)=>{
    const newOtp=otpGenerator();
    storeOtp(username,email,newOtp);
    transporter.sendMail(generatePasswordResetMailOptions(email,newOtp),(error,info)=>{
        if(error){
            console.log(`Error sending email: ${error}`)
            res.status(500).json({message:'Error sending email'})
        }
...
module.exports={deleteUserOTP,userDeleted,welcomeToServer,sendPasswordResetOtp}
